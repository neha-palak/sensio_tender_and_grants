...
instead of dropping the grant. No API key is needed: it runs on the local Claude
...
Env knobs (all optional):
    USE_CLAUDE_FALLBACK=0     disable entirely; restore the old drop-the-grant behaviour
...
# Latched once the CLI turns out to be missing, so we stop retrying per grant.
...
def run_claude(prompt: str) -> str:
    ...
    with _claude_semaphore:
        try:
            ...
            # Latch off — every subsequent grant would fail identically.
