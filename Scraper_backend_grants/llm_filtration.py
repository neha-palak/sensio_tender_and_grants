...
def generate_with_rotation(model, contents, config):
    ...
    keys on a 429 instead of failing the grant outright. Mirrors the
    ...
    rather than failing the grant — set USE_CLAUDE_FALLBACK=0 to opt out.
    """
...
json_file = os.path.join(BASE_DIR, "all_grants.json")

def evaluate_and_score_grants(input_json_path=os.path.join(BASE_DIR, "all_grants.json"), output_json_path=os.path.join(BASE_DIR, "all_grants.json")):

    """
    Reads semantically filtered grants matching the scraper layout, runs them
    ...
    print(f" Successfully loaded {len(grants)} grants from semantic cache.")
    print(" Commencing Multi-Criteria Architectural Analysis using Gemini 2.5 Engine...\n")

    system_instruction = """
You are a strict, precise technical auditing system. Your sole task is to evaluate an incoming grant JSON object against an uncompromising 10.0-Point scoring rubric, calculate an exact composite float score out of 10.0, and append that calculated score to the JSON object under the key "LLM_RelevancyScore".

Do NOT rewrite the grant.
...
- Evaluate the grant EXCLUSIVELY based on the structural criteria provided below.
- You are completely open to custom product development, new product lines, and alternative form factors. Do not penalize a grant for proposing a brand-new type of device or hardware as long as it tracks sensor metrics.
...
* 5.0 Points: Direct Sensor/Tracking Match
  The grant explicitly mandates tracking, measuring, or collecting data using physical hardware, sensors, biometric components, wearables, portable trackers, or data-collection points. This applies across any sector (Health, Defence, Corporate, Pet tracking, or an entirely new tracking domain).
...
* 2.5 Points: Mobile / Wireless Transmission
  The grant requires data to move wirelessly via any modern broadcasting protocols (e.g., Bluetooth/BLE, Wi-Fi, Cellular LTE/5G, Mesh topologies, or Satellite communication) back to a smartphone application, desktop dashboard, cloud ecosystem, or external API endpoints.
...
"""
    ...
    #         "Link to the Grant": types.Schema(type=types.Type.STRING),
    #         "Grant Title": types.Schema(type=types.Type.STRING),
    #         "Grant Description": types.Schema(type=types.Type.STRING),
    ...
    #     # grant_title = grant.get("Grant Title", "Unknown Identifier")
    #     grant_title = str(grant.get("Grant Title") or "Unknown Identifier")
    #     print(f" Processing Record {index}/{len(grants)}: '{grant_title[:45]}...'")
    ...
    for index, grant in enumerate(grants, start=1):
        ...
        grant["Grant Title"] = str(grant.get("Grant Title") or "")
        grant["Grant Description"] = str(grant.get("Grant Description") or "")
        grant["Organisation Name"] = str(grant.get("Organisation Name") or "")

        grant_title = grant["Grant Title"] or "Unknown Identifier"

        print(f" Processing Record {index}/{len(grants)}: '{grant_title[:45]}...'")

        if not grant["Grant Title"]:
            print(f"\n⚠️ Grant #{index} has no title:")
            print(json.dumps(grant, indent=2))

        structured_prompt = (
            f"Please run our 4-pillar architectural assessment matrix against this incoming grant data record. "
            f"Analyze the text and populate the 'LLM_RelevancyScore' float fields:\n\n"
            f"{json.dumps(grant, indent=2)}"
        )
        ...
        try:
            ...
            # Only copy the score into the original grant
            grant["LLM_RelevancyScore"] = score.get(
                "LLM_RelevancyScore",
                0.0
            )

            enriched_dataset.append(grant)

        except Exception as error:
            print(f"  ⚠️ Error encountered while scoring item {index}. Injecting fallback baseline score. Detail: {error}")
            grant["LLM_RelevancyScore"] = 0.0
            enriched_dataset.append(grant)
...
# # PART 1: LLM ANALYTICAL SCORER ENGINE (UNTOUCHED RUBRIC)
# # =====================================================================
# def evaluate_and_score_grants(input_json_path="all_grants.json", output_json_path="all_grants.json"):
#     """
#     Reads semantically filtered grants matching the scraper layout, runs them
#     through the 2-Dimension Technical Fit Rubric via Gemini 2.5, appends
#     the LLM_RelevancyScore, and outputs the exact schema structure to disk.
...
# if __name__ == "__main__":
#     evaluate_and_score_grants(
#         input_json_path="all_grants.json",
#         output_json_path="all_grants.json"
#     )
