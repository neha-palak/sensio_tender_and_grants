...
def clean_grant_string(value_str):
...
def parse_and_convert_to_inr(value_str, assigned_currency_code, live_rates):
    ...
    cleaned_text = clean_grant_string(value_str)
...
# =====================================================================
def json_to_excel(json_filename="all_grants.json", excel_filename="all_grants_pipeline.xlsx"):
    try:
        ...
        for index, grant in enumerate(data, start=1):
            ...
            # explicit_currency = grant.get("Budget Currency", "").strip().upper()
            explicit_currency = str(grant.get("Budget Currency") or "").strip().upper()
            
            if explicit_currency and explicit_currency in live_exchange_rates:
                currency_code = explicit_currency
            else:
                cleaned_budget_str = clean_grant_string(str(min_val_str))
            ...
            opening_date_raw = grant.get("Opening Date") if grant.get("Opening Date") else grant.get("BidOpeningDate", "")
            closing_date_raw = grant.get("Expiry Date") if grant.get("Expiry Date") else grant.get("BidEndDate", "")
            ...
            sector_value = grant.get("Sector")
            if not sector_value or sector_value == "None":
                sector_value = grant.get("Health or Defence Category", "N/A")

            row = {
                "Primary Key": primary_key,
                "Relevancy Score": float(grant.get("LLM_RelevancyScore", 0.0)),
                "Grant Title": grant.get("Grant Title", ""),
                "Description": grant.get("Grant Description", ""),
                "Organisation name": grant.get("Organisation Name", ""),
                "Grant URL": grant.get("Link to the Grant", ""),
                ...
                "Grant Status": resolved_status,
                "Award Date": grant.get("Award Date") if grant.get("Award Date") else grant.get("AwardDate", "N/A"),
        ...
        columns_order = [
            "Primary Key", "Relevancy Score", "Grant Title", "Description",
            "Organisation name", "Grant URL", "Original Currency", "Original Currency Minimum", "Original Currency Maximum",
            "INR Budget Minimum", "INR Budget Maximum", "Sector", "Opening date", "Closing date",
            "Days remaining", "Grant Status", "Award Date", "Country"
        ]
        df = df[columns_order]
        ...
        with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
            ...
            status_col_idx = columns_order.index("Grant Status") + 1
...
if __name__ == "__main__":
    ...
    json_to_excel(
        ...
        excel_filename="all_grants_pipeline.xlsx"
    )
