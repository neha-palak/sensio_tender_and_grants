...
def json_to_excel(json_filename="all_tenders.json", excel_filename="all_tenders_pipeline.xlsx"):
    try:
        ...
        for index, tender in enumerate(data, start=1):
            # Date-stamped, not year+sequential: this run's tenders are meant
            # to be genuinely new rows each week (see write_rows_to_postgres,
            # which replaces the whole tenders table every run), not upserted
            # against a prior run's keys.
            primary_key = f"TND-{run_date}-{index:02d}"
        ...
        write_rows_to_postgres(excel_rows, "tenders", "Tender Title", "Tender URL")
